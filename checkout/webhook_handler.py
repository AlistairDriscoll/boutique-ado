...
class StripeWH_Handler:
    ...
    # setup method that's called everytime an instance of the class is made
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        # Update profile information if save info was selected
        profile = None
        username = intent.metadata.username
        if username != 'AnonymousUser':
            profile = UserProfile.objects.get(user__username=username)
            # bug fix needed here as commas after each line not needed
            # saved info as tuples and not strings
            if save_info:
        ...
        while attempt <= 5:
            try:
                order = Order.objects.get(
                    # iexact means exact match but case insensitive
